...
This document details the Docker Compose configuration options for RealmAI API, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the RealmAI API service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the RealmAI API service itself:
...
### RealmAI API Service Configuration
...
| `environment`    | Environment variable settings, used to configure RealmAI API behavior              |
...
RealmAI API supports various environment variable configurations. Here are some commonly used ones:
