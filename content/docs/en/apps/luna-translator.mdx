...
description: LunaTranslator Tutorial — An open-source GalGame / visual novel translator that supports HOOK, OCR, and AI translation engines. Integrates with RealmAI API to improve translation quality.
---
...
LunaTranslator supports integration with locally deployed RealmAI API. Please use your own deployed RealmAI API, or ensure that the service provider has legitimate upstream authorization and compliance obligations for their RealmAI API service. Do not connect API addresses or keys from unknown sources to production environments.

### One-Click Configuration

1.  In the RealmAI API console's `System Settings` -> `Chat Settings`, add the following shortcut option:
