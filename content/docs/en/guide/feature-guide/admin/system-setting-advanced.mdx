...
Ratio settings are the core configuration of the RealmAI API billing system. By setting different Ratios, you can flexibly control the billing standards for various models and user groups.

### Ratio System Overview

RealmAI API uses a three-tier Ratio system to calculate user Quota consumption:
...
In the RealmAI API system, Ratios are key parameters for calculating Quota consumption. Quota is the internal billing unit of the system, and all API calls are ultimately converted into Quota points for deduction.
...
RealmAI API employs a dual billing mechanism of pre-consumption and post-consumption:
